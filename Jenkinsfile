                ...
                stage("Deploy On AWS EC2 Instance"){
                    steps{
                        ...
                        def remote = [:]
                            ...
                            sshCommand remote: remote, command: "docker container run --env PASSWORD=${password} --env DATABASE=${database} -d -p 443:8443 --name auction ${dockerLogin}/auction"
                        ...
                        waitUntil(initialRecurrencePeriod: 2000) {
                            script {
                                def result =
                                sh script: "curl -k --silent --output /dev/null https://${host}/auction-system-api/api/v1/categories",
                                returnStatus: true
                                return (result == 0)
                            }
                        }
